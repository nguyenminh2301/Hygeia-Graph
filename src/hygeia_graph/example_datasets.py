...
def generate_hard_dataset(seed: int = 42) -> pd.DataFrame:
    ...
    data["SystolicBP"] = (
        120 + data["Age"] * 0.3 + data["BMI"] * 0.5 + np.random.normal(0, 12, n)
    ).clip(90, 200).round()
    data["DiastolicBP"] = (
        data["SystolicBP"] * 0.6 + np.random.normal(0, 8, n)
    ).clip(50, 120).round()
    data["HeartRate"] = np.random.normal(72, 12, n).clip(45, 120).round()
    data["HbA1c"] = (
        5.2 + data["BMI"] * 0.04 + np.random.normal(0, 0.7, n)
    ).clip(4, 13).round(1)
    ...
    data["FatigueScore"] = (
        data["PainScore"] * 0.4 + np.random.normal(4, 2, n)
    ).clip(0, 10).round(1)
    data["AnxietyScore"] = np.random.normal(4, 2.5, n).clip(0, 10).round(1)
    data["DepressionScore"] = (
        data["AnxietyScore"] * 0.5 + np.random.normal(0, 1.5, n)
    ).clip(0, 10).round(1)
    data["InsomniaScore"] = (
        data["AnxietyScore"] * 0.3 + data["PainScore"] * 0.2 + np.random.normal(3, 2, n)
    ).clip(0, 10).round(1)
    ...
    data["MusclePain"] = (
        data["FatigueScore"] * 0.3 + np.random.normal(3, 2, n)
    ).clip(0, 10).round(1)

    # Lifestyle (4)
    data["SmokingStatus"] = np.random.choice(
        ["Never", "Former", "Current"], n, p=[0.45, 0.35, 0.2]
    )
    data["AlcoholUse"] = np.random.choice(
        ["None", "Light", "Moderate", "Heavy"], n, p=[0.25, 0.4, 0.25, 0.1]
    )
